...
public class CreateSpritesIn_Circle2 : MonoBehaviour
{
	...
	void SetupSprites ()
	{
		float degrees = 360.0f / numberOfSprites;
		//	Vector3 center = GetComponent<SpriteRenderer> ().bounds.center;
		Vector3 startPosition = new Vector3 (transform.position.x + 4.5f, transform.position.y + 4.5f, 1.0f);
		...
		for (int i = 0; i < numberOfSprites; i++) {
			...
			spring.anchor = new Vector2 (-0.40f, 0.0f);
			spring.connectedAnchor = hingeJointconnectionOffsettList [i];
