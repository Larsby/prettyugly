Shader "Custom/SoftShadowBump" {
	Properties {
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_BumpMap ("Normal", 2D) = "bump" {}
		_Magnitude ("Normal Magnitude", Range(0,1)) = 0.05
		...
		Pass {
			...
			CGPROGRAM
				...
				fixed4 FShader(VertexOutput v) : SV_Target {
					half4 bump = tex2D(_BumpMap, v.texCoord);
					half2 distortion = UnpackNormal(bump).rgb;
				//	v.texCoord.xy += distortion * _Magnitude;
					float2 coord = v.texCoord.xy += distortion * _Magnitude;
				//	fixed4 color = tex2Dproj( _MainTex, UNITY_PROJ_COORD(v.texCoord));
					fixed4 color = v.color*tex2D(_MainTex, coord);
