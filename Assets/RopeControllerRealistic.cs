...
public class RopeControllerRealistic : MonoBehaviour 
{
	...
	private float mRopeSection = 0.1f;
	public float startWidth = 0.2f;
	public float endWidth = 0.2f;
	public int ropeSectionSize = 14;
	public Vector3 gravityForce = new Vector3(4.2f, -4.21f, 0f);
	public bool alternateTarget = false;
	public float startAngle;
	EdgeCollider2D 	collider = null;
	public bool useCollider = true;
	private bool calculateColliders = false;
	public bool useShadow = false;
	public bool isPlayer = true;
	void RecalcCollider(EdgeCollider2D collider) {
		List<Vector2> colliderList = new List<Vector2>();
		for (int i = 0; i < allRopeSections.Count; i++)
		{

			//Vector3 worldSpace = Camera.main.WorldToViewportPoint (allRopeSections [i].pos);
			Vector2 pos = new Vector2 (allRopeSections [i].pos.x, allRopeSections [i].pos.y);
			Vector3 local = transform.InverseTransformPoint(allRopeSections [i].pos);
				pos = new Vector2 (local.x,local.y);
			colliderList.Add (pos);
		}
		collider.points = colliderList.ToArray();
		collider.isTrigger = PlayerController.instance.IsTrigger ();
	}
	Vector2[] GetShadowPointLists() {
		List<Vector2> colliderList = new List<Vector2>();
		for (int i = 0; i < allRopeSections.Count; i++)
		{
			...
			Vector2 pos = new Vector2 (allRopeSections [i].pos.x, allRopeSections [i].pos.y);
			Vector3 local = transform.InverseTransformPoint(allRopeSections [i].pos);
			pos = new Vector2 (local.x-0.1f,local.y-0.1f);
			colliderList.Add (pos);
			//pos = new Vector2 (local.x+0.5f,local.y);
			//colliderList.Add (pos);
		}
	...
	void SetUpCollider() {
		...
		if (collider != null) {
			...
			RecalcCollider (collider);
			calculateColliders = true;
	...
	void Start() 
	{
		...
		lineRenderer.sortingLayerName = "Tentacle";
	
		//
		//Create the rope
		//
		//Build the rope from the top
		Vector3 pos  = getPoint(whatTheRopeIsConnectedTo.position,startAngle,0.3f);
		/*
		 * Vector3 pointA = getPoint (whatIsHangingFromTheRope.position, startAngle, 0.2f);
		Vector3 pointB = getPoint(whatTheRopeIsConnectedTo.position,startAngle,0.3f);
		 * */

		List<Vector3> ropePositions = new List<Vector3>();

		for (int i = 0; i <ropeSectionSize; i++)
		{
	...
	void LockAim(float steps) {
		
		Vector3 pointA = getPoint (PlayerController.instance.getTargetPosition(), startAngle, PlayerController.instance.innerRadius);
		Vector3 pointB = getPoint(whatTheRopeIsConnectedTo.position,startAngle,PlayerController.instance.outerRadius);


		Vector3 current = pointA;
		//draw.SetPosition (0, pointA);
		Vector3 step = pointB - pointA;
		int segements = allRopeSections.Count - 1;
		step = new Vector3 (step.x /segements  , step.y/segements, 0.0f);
		Vector3 iterator = pointA-step;

		for (int i = 0; i < segements; i++) {
			RopeSection thisRopeSection = allRopeSections [i];

			iterator = iterator + step;
			Vector3 newPos = Vector3.zero;
			if (PlayerController.instance.immediate) {
				newPos = iterator;
			} else {
				newPos = Vector3.Lerp (allRopeSections [i].pos, iterator + step, steps );
			}
			thisRopeSection.pos = newPos;
			allRopeSections [i] = thisRopeSection;

		}
		RopeSection lastSection = RopeSection.zero;


		lastSection.pos = pointB;
		allRopeSections [segements] = lastSection;
	...
	void FixedUpdate()
	{
		...
		if (PlayerController.instance.isStretchMode() == false || isPlayer ==false) {
			...
			float timeStep = Time.fixedDeltaTime / (float)1;
			LockAim (timeStep);
		...
		if (useShadow) {
			GetComponent<SFPolygon> ().SetVerts (GetShadowPointLists(),Vector2.zero,false);
		}
	}
	...
	private void UpdateRopeSimulation(List<RopeSection> allRopeSections, float timeStep)
	{
		...
		lastRopeSection.pos =  getPoint(whatTheRopeIsConnectedTo.position,startAngle,PlayerController.instance.outerRadius);
		...
		for (int i = 0; i < allRopeSections.Count - 1; i++)
		{
			...
			//pos = pos + vel * t
			thisRopeSection.pos = allRopeSections[i].pos + allRopeSections[i].vel * timeStep *0.2f;
	...
	private List<Vector3> CalculateAccelerations(List<RopeSection> allRopeSections)
	{
		...
		for (int i = 0; i < allRopeSections.Count - 1; i++)
		{
			...
			//Force from gravity
			Vector3 _gravityForce = springMass * gravityForce;

			//The total force on this spring
			Vector3 totalForce = springForce + _gravityForce - dampingForce;
